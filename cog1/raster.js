...
function(exports, shader, framebuffer, data) {
	...
	function drawLineBresenham(startX, startY, startZ, endX, endY, endZ, color, storeIntersectionForScanlineFill, edgeStartVertexIndex, edgeEndVertexIndex, edgeStartTextureCoord, edgeEndTextureCoord) {
		...
		// fallunterscheidung deltaX >= deltaY
		// was ist die treibende variable ?
		if (dXAbs >= dYAbs) 
			{
				// x is driving variable.
				// dann ist x treibende variable
				// e1 (fehler) = absoluter betrag deltaX - 2* absoluter betrag deltaY
				e = dXAbs - dYAbs2;

				// Schleifenbeginn: solange xn != xend (abbruch wenn endpunkt erreicht)
				// der startpunkt wird gezaehlt wenn er endpunkt ist - ist ende im gelaende

				while (startX != endX)
					{
					
					// n = n+1 
					// zaehlervariable die mich verwirrt ... 
					// startX++; ?? 
 
 					// xn = xn-1 + sign(deltaX)
 					// wieder zaehlervariable ... 

					startX = startX + dXSign;
					

					// fallunterscheidung en > 0
					// wenn fehler größer null dann bleibt y erhalten weil der errechnete fehlerpunkt ober-/unterhalb der geraden ist
					// und wenn y hochgezaehlt wird - dann ist es noch weiter entfernt von der geraden als der jetzige (-> video bresenham)
					if (e > 0)
						{
							//yn = yn - 1 
							//jetziger y-punkt ist gleich dem y punkt vom vorherigen durchlauf 
							// y bleibt gleich 
							startY = startY ;

							// en+1 = en - 2 * absoluter betrag (deltaY)
							// neuer fehlerpunkt (orientierungspunkt)  wird errechnet - der muss ja weiterspringen
							e = e - dYAbs2;
							
						}
					
					// hier ist e < 0 
					// also der punkt liegt oberhalb/unterhalb ?! somit wäre der hochgezählte y-punkt näher dran
						...
						
						{
							...
							// yn = yn-1 +sign(deltaY)
							// dYsign gibt die richtung vor - 1 pixel hoch oder runter (+1 oder -1) - steigung der geraden ?!?
							// y-punkt wird neu errechnet ... 

							startY = startY + dYSign;
							
							// en+1 = en + 2*(abs(deltaX)-abs(deltaY))
							// neuer fehlerpunkt (orientierungspunkt)  wird errechnet - der muss ja weiterspringen
							e = e + dXdYdiff2;
							
						}

					// wenn feststeht welche koordinaten errechnet wurden - werden jene an framebuffer uebergeben .. 
					// also hier die set Pixel ???

					framebuffer.set(startX, startY, getZ(x, y), color);
					}	
			}
		...
		else	
			{
				...
				// dann ist y treibende variable
				// e1 (fehler) = absoluter betrag deltaY - 2* absoluter betrag deltaX
				e = dYAbs - dXAbs2;
				...
				while (startY != endY)
					...
					// fallunterscheidung en > 0
					// wenn fehler größer null dann bleibt x erhalten weil der errechnete fehlerpunkt ober-/unterhalb der geraden ist
					// und wenn x hochgezaehlt wird - dann ist es noch weiter entfernt von der geraden als der jetzige (-> video bresenham)
					if (e > 0)
						{
							//xn = xn - 1 
							//jetziger y-punkt ist gleich dem y punkt vom vorherigen durchlauf 
							...
							// en+1 = en - 2 * absoluter betrag (deltaX)
							// neuer fehlerpunkt (orientierungspunkt)  wird errechnet - der muss ja weiterspringen
							e = e - dXAbs2;
							
						}
					// hier ist e < 0 
					// also der punkt liegt oberhalb/unterhalb ?! somit wäre der hochgezählte x-punkt näher dran

					else
						{

							// xn = xn-1 +sign(deltaX)
							// dXsign gibt die richtung vor - 1 pixel hoch oder runter (+1 oder -1) - Steigung der geraden ?!?
							// x-punkt wird neu errechnet ... 
							startX = startX + dXSign;


							// en+1 = en + 2*(abs(deltaY)-abs(deltaX))
							// neuer fehlerpunkt (orientierungspunkt)  wird errechnet - der muss ja weiterspringen
							e = e + dYdXdiff2;
		...
		// ende der funktion - dann wieder neuaufruf fuer neue gerade ... 
		return;
